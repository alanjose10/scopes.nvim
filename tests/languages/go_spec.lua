...
describe("languages.go", function()
  ...
  describe("no overlap between scope_types and symbol_types", function()
    it("has no types in both lists", function()
      for _, st in ipairs(go.scope_types) do
        assert.is_false(vim.tbl_contains(go.symbol_types, st), st .. " is in both scope_types and symbol_types")
      end
    end)
  end)
  ...
  describe("structural checks", function()
    it("passes all structural invariants", function()
      helpers.assert_valid_lang_config(go)
    end)
  end)
  ...
  describe("get_name", function()
    ...
    it("extracts method_declaration names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "method_declaration")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("returns [anonymous] for func_literal", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "func_literal")
    ...
    it("extracts type_declaration names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "type_declaration")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("extracts const_spec names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "const_spec")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("extracts var_spec names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "var_spec")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("extracts field_declaration names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "field_declaration")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("returns 'import' for import_declaration", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "import_declaration")
    ...
    it("extracts import_spec path names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "import_spec")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
    ...
    it("extracts short_var_declaration names", function()
      local root = get_root()
      local nodes = helpers.find_ts_nodes(root, "short_var_declaration")
      local names = {}
      for _, node in ipairs(nodes) do
        table.insert(names, go.get_name(node, bufnr))
      end
